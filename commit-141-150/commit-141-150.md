...
注意哈，之前处理css是交给`rollup-plugin-vue`处理的，[使用`transform`处理`css`](https://github.com/vuejs/rollup-plugin-vue/blob/next/src/index.ts)，这个和我们`dev`环境下是一样的。

这里做的事情是**内容与文件名称生成一个bundle**。
