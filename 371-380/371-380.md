...
>
> 对所有开放etag，`sw.js`被`serverPluginServiceWorker.ts`拦截；影响到的只有`-sw`重置去掉`true`，`serverPluginVue.ts` | `serverPluginResolve` | `serverPluginEsbuild` 使用`cachedRead`方法，对比`etag`就能直接触发304。
