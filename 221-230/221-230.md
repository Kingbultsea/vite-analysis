...
假如现在有组件`Comp.vue`，执行`rollup-plugin-vue`的`transform`钩子，转换三大块标签为`import`资源，经手`load`钩子返回`script`原内容，然后`buildPluginEsbuild`的`transform`钩子识别到`lang=ts`，进行转换。
...
> 我理解的可以使用custom事件来定义一下（或者我直接改源码吧）
