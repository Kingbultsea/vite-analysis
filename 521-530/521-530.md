...
# 524 - c493629 [#227](https://github.com/vitejs/vite/issues/227) 支持条件导出
...
> [conditional exports](https://nodejs.org/api/packages.html#conditional-exports)：条件导出，

### 改动一 支持exports字段下没有`package.json`的包
...
如果没有`package.json`，则默认返回`pkg1`，即被改写为`@modules/pkg1`，后续在`moduleResolve`下会调用`resolveNodeModuleFile`处理路径，即`require.resolve('pkg1', { paths: [root] })`。
...
# 528 - 7a3e822 [#230](https://github.com/vitejs/vite/pull/230) 支持引入相对路径下的包

在工作目录创建一个test包，`package.json`入口为`index2.js`，设置`link:./test`，引入：

![4](4.png)

```typescript
import * as i from 'test'
```

如果不改动的情况下，`moduleRewrite`，改写为`'/@modules/test/index2.js'`。

现在需要支持这个功能，可以在`resolver.requestToFile`动手去寻找
